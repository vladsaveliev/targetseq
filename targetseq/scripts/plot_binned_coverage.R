...
write(sprintf("plot_binned_coverage.R\n"))
...
} else
{
     ...
     for (i in 1:length(bcov$counts)){
          if (bcov$counts[i] > 0.005 * ymax){
               plotmax = i
          }
     }
     ...
     bdata <- barplot( bcov$counts[1:plotmax], names.arg=bcov$read_depth[1:plotmax], ylim=c(0,ymax),
          xlab="Read Depth", ylab="Bases Covered at Read Depth", main="Target Coverage" )
     ydata = bcov$pc_cum_counts[1:plotmax] * 0.996 * ymax / 100
     lines( bdata, ydata, type='o', col="blue", lwd=2 )
     ryax=(0:10)*(ymax/10)
     rlab=paste(((0:10)*10),"%",sep="")
     axis(4,at=ryax,labels=rlab,tck=-0.02,las = 1,col = "blue")
