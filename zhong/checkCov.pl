...
use lib '/gpfs/users/kdld047/lib/perl5';
use Getopt::Std;
use Stat::Basic;
use strict;
...
while( <> ) {
    ...
    my @starts = split(/,/, $A[$s_col]); # took all start positions
    my @ends = split(/,/, $A[$e_col]); #took all end positions
    my @CDS = ();
    my @pos = ();
...
while( my ($gene, $r) = each %regions ) {
    ...
    for(my $i = 0; $i < @{ $CDS }; $i++) {
        ...
        while( <SAM> ) {
            ...
            my $start = $a[3];
            my $n = 0;
            my $dir = $a[1] & 0x10 ? "-" : "+"; #AB is it ?????
            while( $a[5] =~ /(\d+)([A-Z])/g ) { #79M
                my $m = $1;
                my $C = $2;
                if ( $C eq "N" ) {
                    $start += $m;
